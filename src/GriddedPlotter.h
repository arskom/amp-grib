...
public:
    ...
    virtual time_t getClosestDateFromNow() const { return isReaderOk() ? getReader()->getClosestDateFromNow() : 0; }
    ...
    virtual bool hasData(const DataCode &dtc) const { return isReaderOk() && getReader()->hasData(dtc); }
    virtual bool hasData(int dataType, int levelType, int levelValue) const { return hasData(DataCode(dataType, levelType, levelValue)); }
    virtual bool hasData(int dataType, const Altitude &alt) const { return hasData(DataCode(dataType, alt.levelType, alt.levelValue)); }

    virtual bool hasDataType(int dataType) const { return isReaderOk() && getReader()->hasDataType(dataType); }
    ...
    // Drawing parameters
    //----------------------------------------------------------------
    virtual void setFastInterpolation(bool b) { fastInterpolation = b; }

    virtual void setInterpolateValues(bool b) { mustInterpolateValues = b; }
    virtual void setWindArrowsOnGrid(bool b) { drawWindArrowsOnGrid = b; }
    virtual void setCurrentArrowsOnGrid(bool b) { drawCurrentArrowsOnGrid = b; }
    virtual void duplicateFirstCumulativeRecord(bool b) { mustDuplicateFirstCumulativeRecord = b; }
    virtual void duplicateMissingWaveRecords(bool b) { mustDuplicateMissingWaveRecords = b; }
    virtual void setUseJetStreamColorMap(bool b) { useJetStreamColorMap = b; }
    virtual Altitude getWindAltitude() { return windAltitude; }

    virtual Altitude getCurrentAltitude() { return currentAltitude; }
