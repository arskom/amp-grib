...
bool CurveDrawer::createTable(const int &iDataChosen) {
    ...
    if (!qvHoursFromNow.isEmpty()) {
        qvHoursFromNow.clear();
    };
    if (!qvWindDir.isEmpty()) {
        qvWindDir.clear();
    };
    if (!qvWindDev.isEmpty()) {
        qvWindDev.clear();
    };
    if (!qvTempMin.isEmpty()) {
        qvTempMin.clear();
    };
    if (!qvTempMax.isEmpty()) {
        qvTempMax.clear();
    };
...
bool CurveDrawer::privateConstructor() {
    ...
    if (reader->hasDataType(GRB_WIND_VX)) {
        myToolBar->cbData->addItem(tr("Wind"), GRB_WIND_SPEED);
    };
    if (reader->hasDataType(GRB_PRESSURE_MSL)) {
        myToolBar->cbData->addItem(tr("Pressure"), GRB_PRESSURE_MSL);
    };
    if (reader->hasDataType(GRB_TEMP)) {
        myToolBar->cbData->addItem(tr("Temperature"), GRB_TEMP);
    };
    if (reader->hasDataType(GRB_CLOUD_TOT)) {
        myToolBar->cbData->addItem(tr("Cloud cover"), GRB_CLOUD_TOT);
    };
    if (reader->hasDataType(GRB_PRECIP_RATE)) {
        myToolBar->cbData->addItem(tr("Precipitation Rate"), GRB_PRECIP_RATE);
    };
    if (reader->hasDataType(GRB_PRECIP_TOT)) {
        myToolBar->cbData->addItem(tr("Precipitation Total"), GRB_PRECIP_TOT);
    };
