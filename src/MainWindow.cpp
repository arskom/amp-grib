...
void MainWindow::InitActionsStatus() {
    ...
    earth->setDrawGeopotential(geopdraw);
    earth->setDrawGeopotentialLabels(glabels);
    earth->setGeopotentialStep(gstep);
    ...
    earth->setWaveArrowsType(Util::getSetting("waveArrowsType", GRB_TYPE_NOT_DEFINED).toInt());

    menuBar->setIsobarsStep(Util::getSetting("isobarsStep", 2).toInt());
    earth->setIsobarsStep(Util::getSetting("isobarsStep", 2).toInt());
    ...
    earth->setIsotherms0Step(Util::getSetting("isotherms0Step", 100).toInt());
    ...
    earth->setIsotherms_Step(Util::getSetting("isotherms_Step", 2).toDouble());
    ...
    earth->setLinesThetaE_Step(Util::getSetting("linesThetaE_Step", 2).toDouble());
    ...
    earth->setCitiesNamesLevel(Util::getSetting("showCitiesNamesLevel", 0).toInt());

    menuBar->acMap_ShowPOIs->setChecked(Util::getSetting("showPOIs", true).toBool());
    earth->setShowPOIs(Util::getSetting("showPOIs", true).toBool());
    ...
    for (int qual = 4; qual >= 0; qual--) {
        if (!gshhsReader->gshhsFilesExists(qual)) {
            switch (qual) {
            case 0:
                menuBar->acMap_Quality1->setEnabled(false);
                break;
            case 1:
                menuBar->acMap_Quality2->setEnabled(false);
                break;
            case 2:
                menuBar->acMap_Quality3->setEnabled(false);
                break;
            case 3:
                menuBar->acMap_Quality4->setEnabled(false);
                break;
            case 4:
    ...
    earth->setMapQuality(quality);
    //-------------------------------------
    earth->setSpecialZone(Util::getSetting("specialZone_x0", 0).toDouble(),
...
void MainWindow::connectSignals() {
    ...
    connect(mb->acMBlue_fastInterpolation, SIGNAL(triggered(bool)),
            earth, SLOT(setMBlueFastInterpolation(bool)));
    ...
    connect(mb->acView_IsobarsLabels, SIGNAL(triggered(bool)),
            earth, SLOT(setDrawIsobarsLabels(bool)));

    connect(mb->acView_Isotherms0, SIGNAL(triggered(bool)),
            earth, SLOT(setDrawIsotherms0(bool)));
    ...
    connect(mb->acView_Isotherms0Labels, SIGNAL(triggered(bool)),
            earth, SLOT(setDrawIsotherms0Labels(bool)));
    ...
    connect(mb->acView_Isotherms_Labels, SIGNAL(triggered(bool)),
            earth, SLOT(setDrawIsotherms_Labels(bool)));
    ...
    connect(mb->acView_LinesThetaE_Labels, SIGNAL(triggered(bool)),
            earth, SLOT(setDrawLinesThetaE_Labels(bool)));

    //-------------------------------------------------------
    connect(mb->acView_GribGrid, SIGNAL(triggered(bool)),
            earth, SLOT(setGribGrid(bool)));
    ...
    connect(dialogUnits, SIGNAL(accepted()), earth, SLOT(slotMustRedraw()));
    ...
    connect(dialogGraphicsParams, SIGNAL(accepted()), earth, SLOT(updateGraphicsParameters()));
    ...
    connect(this, SIGNAL(signalMapQuality(int)),
            earth, SLOT(setMapQuality(int)));
    //-----------------------------------------------------------
    connect(earth, SIGNAL(mouseClicked(QMouseEvent *)),
            this, SLOT(slotMouseClicked(QMouseEvent *)));
    connect(earth, SIGNAL(mouseMoved(QMouseEvent *)),
            this, SLOT(slotMouseMoved(QMouseEvent *)));
    connect(earth, SIGNAL(mouseLeave(QEvent *)),
    ...
    connect(mb->acAlt_GeopotLabels, SIGNAL(triggered(bool)),
            earth, SLOT(setDrawGeopotentialLabels(bool)));
    //-----------------------------------------------------------
...
MainWindow::MainWindow(int w, int h, bool withmblue, QWidget *parent)
    ...
    //--------------------------------------------------
    earth = new Terrain(this, proj, gshhsReader);
    assert(earth);
...
void MainWindow::slotTimeZoneChanged() {
    if (earth->getGriddedPlotter()->isReaderOk()) {
        menuBar->updateListeDates(
                earth->getGriddedPlotter()->getListDates(),
...
void MainWindow::createPOIs() {
    ...
    for (int i = 0; i < lscodes.size(); ++i) {
        ...
        poi = new POI(code, proj, this, earth);
        connectPOI(poi);
    }
}
//-------------------------------------------------
...
void MainWindow::createAllMETARs() {
    ...
    QStringList allMetarsSelected = (Util::getSetting("metar_selected", QStringList()).toStringList());
    ...
    if (allMetarsSelected.size() > 0) {
        ...
        for (int i = 0; i < allMetarsSelected.size(); i++) {
            ...
            mw = factory->createMetarWidget(icao, isVisible, proj, earth);
...
void MainWindow::slotOpenCurveDrawer() {
    ...
    if (GLOB_listSelectedPOI.empty()) {
        proj->screen2map(mouseClicX, mouseClicY, &lon, &lat);
        new CurveDrawer(earth->getGriddedPlotter(), lon, lat);
    }
    else if (GLOB_listSelectedPOI.size() == 1) {
        new CurveDrawer(earth->getGriddedPlotter(), GLOB_listSelectedPOI.first()->getLongitude(),
...
void MainWindow::slotGroupIsotherms(QAction *ac) {
    ...
    if (ac && ac->isChecked()) {
        ...
        earth->setIsotherms_Altitude(alt);
        earth->setDrawIsotherms(true);
...
void MainWindow::slotGroupLinesThetaE(QAction *ac) {
    ...
    if (ac) {
        ...
        earth->setLinesThetaE_Altitude(alt);
        earth->setDrawLinesThetaE(true);
        //DBGQS ("MainWindow Alt = "+AltitudeStr::toString (alt));
    }
    else {
        earth->setDrawLinesThetaE(false);
    }
}
...
void MainWindow::slotFile_MBLUE_Load() {
    ...
    if (earth->getSelectedRectangle(&x0, &y0, &x1, &y1)
            || earth->getGribFileRectangle(&x0, &y0, &x1, &y1)) {
...
void MainWindow::slotFile_MBLUE_ShowArea() {
    ...
    if (bt == menuBar->acMBlueSwiss_ShowArea) {
        if (bt->isChecked()) {
            double x0, y0, x1, y1;
            if (MblueReader::getMeteoblueTotalArea(MBLUE_SWISS, &x0, &y0, &x1, &y1)) {
                earth->setSpecialZone(x0, y0, x1, y1);
                earth->showSpecialZone(true);
                earth->zoomOnZone(x0, y0, x1, y1);
            }
            ...
            earth->showSpecialZone(false);
            earth->slotMustRedraw();
        }
    }
}
...
void MainWindow::slotFile_Info_GRIB() {
    ...
    if (record->isRegularGrid()) {
        ...
        msg += QString(" : %1°x%2°")
                       .arg(record->getDeltaX())
                       .arg(record->getDeltaY());
        msg += "\n";
    }
    else {
...
void MainWindow::statusBar_showSelectedZone() {
    ...
    QString message = tr("Selected area: ")
...
void MainWindow::updateBoardPanel() {
    if (earth->underMouse()) {
        ...
        double xx, yy;
        int x, y;
        earth->getLastMousePosition(&x, &y);
...
void MainWindow::slot_GroupColorMap(QAction *act) {
    ...
    MenuBar *mb = menuBar;
    GriddedReader *reader = earth->getGriddedPlotter()->getReader();
    DataCode dtc, dtctmp;
...
void MainWindow::setMenuBarGeopotentialLines(
    ...
    switch (step) {
    case 1:
        mb->acAlt_GeopotStep_1->setChecked(true);
        break;
    case 2:
        mb->acAlt_GeopotStep_2->setChecked(true);
        break;
    case 5:
        mb->acAlt_GeopotStep_5->setChecked(true);
        break;
    case 10:
        mb->acAlt_GeopotStep_10->setChecked(true);
        break;
    case 20:
        mb->acAlt_GeopotStep_20->setChecked(true);
        break;
    case 50:
        mb->acAlt_GeopotStep_50->setChecked(true);
        break;
    case 100:
        mb->acAlt_GeopotStep_100->setChecked(true);
        break;
    }
...
void MainWindow::slot_GroupGeopotentialLines(QAction *act) {
    ...
    if (dtc.dataType != GRB_TYPE_NOT_DEFINED) {
        earth->setGeopotentialData(dtc);
        earth->setDrawGeopotential(act->isChecked());
    }
    else {
...
void MainWindow::openSkewtDiagramWindow(double lon, double lat,
    ...
    if (reader == NULL) {
        ...
        reader = earth->getGriddedPlotter()->getReader();
        date = earth->getGriddedPlotter()->getCurrentDate();
    }
