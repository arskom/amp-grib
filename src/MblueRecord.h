...
public:
    ...
    std::map<uint32_t, float> *getMapValues() { return &mapvalues; }
...
public:
    ...
    virtual std::string getDataOrigin() const { return "Meteoblue-NMM"; }

    void addMbluePoint(MbluePoint *pt);
    void finalize(); // don't forget me
    bool isOk() const { return ok; }
    void setFastInterpolation(bool b) { fastInterpolation = b; }
    ...
    double getSmoothPressureMSL(int i, int j) const { return smoothPressureGrid[i + j * Ni]; }
    ...
    virtual int getTotalNumberOfPoints() const { return ok ? allPoints.size() : 0; }
    virtual double getAveragePointsDensity() const { return ok ? allPoints.size() / ((xmax - xmin) * (ymax - ymin)) : 0; }
