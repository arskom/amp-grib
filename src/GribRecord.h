...
public:
    ...
    bool isOrientationAmbiguous() const { return verticalOrientationIsAmbiguous; }
    bool isWaveData() { return waveData; }
    //-----------------------------------------
    DataCode getDataCode() const { return DataCode(dataType, levelType, levelValue); }
    ...
    virtual int getTotalNumberOfPoints() const { return ok ? Ni * Nj : 0; }
    virtual double getAveragePointsDensity() const { return ok ? Ni * Nj / ((xmax - xmin) * (-ymin)) : 0; }
