...
#define DBGQRect(blabla, r)                                                                                                                                                                                                    \
    {                                                                                                                                                                                                                          \
        fprintf(stderr, "DBG %s: %d: %s(): ", __FILE__, __LINE__, __FUNCTION__);                                                                                                                                               \
        fprintf(stderr, "%s X0=%g %g   X1=%g %g    size=%g %g\n", qPrintable(blabla), (double)(r.x()), (double)(r.y()), (double)(r.x() + r.width()), (double)(r.y() + r.height()), (double)(r.width()), (double)(r.height())); \
    }
#else
#define DBG(...)
#define DBGN(...)
...
public:
    ...
    //-------------------------------------------------
    template <typename T>
    static bool isInRange(T v, T min, T max) { return (v >= min && v <= max); }

    static bool isInRange(double v, double min, double max) { return (v >= min && v <= max); }
