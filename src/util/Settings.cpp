...
void Settings::initializeSettingsDir() {
    ...
    if (path == "") { // try zygrib_directory/config
        dir = QDir::current();
        if (dir.exists(Util::pathGshhs())
                && dir.exists(Util::pathGis())) { // seem's zygrib directory, try yo write a file
...
QList<uint> Settings::getSettingAllCodesPOIs() {
    ...
    else { // try to load from native settings
...
void Settings::initializeGribFilesDir() {
    ...
    if (path == "") { // try zygrib_directory/grib
        dir = QDir::current();
        if (dir.exists("maps") && dir.exists("img")) { // seem's zygrib directory, try yo write a file
            if (Util::isDirWritable(dir)) {
