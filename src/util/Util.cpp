...
#ifdef Q_OS_MACX
    if (QSysInfo::MacintoshVersion > QSysInfo::MV_10_8) { // fix Mac OS X 10.9 (mavericks) QFileDialog bug
...
void Util::setApplicationProxy() {
    ...
    if (Util::getSetting("httpUseProxy", false).toBool()) {
        proxy.setType((QNetworkProxy::ProxyType)(Util::getSetting("httpProxyType", QNetworkProxy::HttpProxy).toInt()));
