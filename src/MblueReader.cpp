...
void MblueReader::openFile(const std::string fname, bool fastInterpolation) {
    ...
    for (uint32_t i = 0; taskProgress->continueDownload && i < nblines; i++) {
        ...
        time_t href = DataRecordAbstract::UTC_mktime(mbzfile.year, mbzfile.month, mbzfile.day, mbzfile.href, 0, 0);
        ...
        if (rec == NULL) {
            ...
            assert(rec);
            mapRecords.insert(std::pair<time_t, MblueRecord *>(hcur, rec));
        }
